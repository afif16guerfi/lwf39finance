...
async function buildExcelReport({ report, settings, query = {} }) {
  ...
  // 9 columns now (اللجنة وجهة الصرف منفصلتان تمامًا — قسم "ثامنًا" من
  // برنامج التحديث): رقم العملية، التاريخ، الوقت، عنوان العملية، نوع
  // العملية، اللجنة، جهة الصرف، المبلغ، الرصيد بعد العملية.
  ...
  const headerRow = sheet.addRow(["رقم العملية", "التاريخ", "الوقت", "عنوان العملية", "نوع العملية", "اللجنة", "جهة الصرف", "المبلغ", "الرصيد بعد العملية"]);
  ...
  report.transactions.forEach((t, i) => {
    const row = sheet.addRow([
      // رقم العملية = الترقيم التسلسلي للعرض (1 → 2 → 3 → ...) في الترتيب
      // الحالي للتقرير، وليس الرقم المرجعي الدائم للعملية.
      t.displayNumber != null ? t.displayNumber : i + 1,
      t.date,
      t.time || "00:00",
      t.title,
      t.type === "income" ? "دخول" : "خروج",
      t.categoryId ? (t.categoryName || "—") : "—",
      t.payeeId ? (t.payeeName || "—") : "—",
      t.amountCents / 100,
      (t.runningBalanceCents ?? 0) / 100,
    ]);
    row.getCell(8).numFmt = "#,##0.00";
    row.getCell(9).numFmt = "#,##0.00";
    if (t.type === "expense") row.getCell(8).font = { color: { argb: "FFC8122E" } };
    else row.getCell(8).font = { color: { argb: "FF0B6E4F" } };
  });

  sheet.addRow([]);
  const totalIncomeRow = sheet.addRow(["", "", "", "", "", "", "إجمالي المداخيل", report.totalIncomeCents / 100]);
  totalIncomeRow.getCell(8).numFmt = "#,##0.00";
  totalIncomeRow.font = { bold: true };
  const totalExpenseRow = sheet.addRow(["", "", "", "", "", "", "إجمالي المصاريف", report.totalExpenseCents / 100]);
  totalExpenseRow.getCell(8).numFmt = "#,##0.00";
  ...
  sheet.columns = [
    { width: 14 }, { width: 12 }, { width: 10 }, { width: 26 }, { width: 10 }, { width: 18 }, { width: 18 }, { width: 16 }, { width: 18 },
  ];

  // إجمالي المصاريف حسب اللجنة — مستقل تمامًا عن حسب جهة الصرف أدناه.
  if (report.byCategory && report.byCategory.length) {
    sheet.addRow([]);
    const catHeader = sheet.addRow(["إجمالي المصاريف حسب اللجنة"]);
    catHeader.font = { bold: true };
    const catCols = sheet.addRow(["اللجنة", "عدد العمليات", "الإجمالي"]);
  ...
  // إجمالي المصاريف حسب جهة الصرف — قسم مستقل تمامًا (لا يُدمج أبدًا مع
  // إجمالي المصاريف حسب اللجنة أعلاه — قسم "ثامنًا"/"تاسعًا" من برنامج
  // التحديث).
  if (report.byPayee && report.byPayee.length) {
    sheet.addRow([]);
    const payeeHeader = sheet.addRow(["إجمالي المصاريف حسب جهة الصرف"]);
    payeeHeader.font = { bold: true };
    const payeeCols = sheet.addRow(["جهة الصرف", "عدد العمليات", "الإجمالي"]);
    payeeCols.font = { bold: true };
    report.byPayee.forEach((p) => {
      const row = sheet.addRow([p.payeeName, p.count, p.totalCents / 100]);
      row.getCell(3).numFmt = "#,##0.00";
    });
  }
